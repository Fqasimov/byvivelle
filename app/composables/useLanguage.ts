...
/**
 * Language state lives in `useState` so it is shared across every component
 * and serialised through SSR. English renders first on both sides; a stored
 * preference is applied after hydration, which keeps the markup identical.
 */
export function useLanguage() {
  const lang = useState<Lang>("byvivelle:lang", () => "en");
  ...
  function setLang(next: Lang) {
    ...
    try {
      ...
      // Private mode / blocked storage — preference just will not persist.
    }
  }

  /** Called once, client-side, from the app root. */
  function restoreLang() {
    try {
      ...
      // Fall through to locale sniffing.
    }

    const nav = navigator.language.slice(0, 2).toLowerCase();
    if (nav === "ru" || nav === "az") lang.value = nav;
  }

  return { lang, setLang, restoreLang, t };
}
